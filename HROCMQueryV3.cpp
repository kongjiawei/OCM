...
DWORD WINAPI ThreadProcScan(LPVOID lpParameter)
{
	...
	while (1)
	{
		...
		//commandITU((int)(191.4 * OCM3_FSCALE + 0.5), (int)( 50* OCM3_FSCALE + 0.5), 80);
		//FinisarHROCM_V3 OCM(theConfigString, theLogFile, theLogBinFile);
		...
		//		printf("%.7f,%.1f,%.1f\n", pThreadData->centerfre[k], pThreadData->power_dbm[k], pThreadData->osnr_db[k]);
		//	}
		//}
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	int Result = 0;
	 {
	   RecvData *myRecvData = (RecvData*)malloc(sizeof(RecvData));
	 ...
	 //初始化WSA
	   ...
	   sin.sin_addr.S_un.S_addr = inet_addr("192.168.109.118");
	   ...
	   while (true)
	   {
		...
		/*   float fstart = myNode->frestart;
		   float fstep = myNode->frestep;
		   int nchannel = myNode->numchannel;
		   printf("fstart:%.1f \t  fstep:%.2f\t nchannel:%d \n",fstart, fstep, nchannel);
		   if (nchannel == 1024)
		   {
			   break;
		   }*/
		   ...
		   //for (int k = 0; k <2; k++) 
		   //{
			  // myData->centerfre[k] = 191.1234567; // Slice numbers are 1-based, not 0-based
			  // myData->power_dbm[k] = -41.5;
			  // myData->osnr_db[k] = -10.5;
		   //}
		  /* char * sendData = "ewrwerwer";
		   send(sClient, sendData, strlen(sendData), 0);*/

		   int needSend = sizeof(Data);
		   char *sendbuffer = (char*)malloc(needSend);
		   
		   //读入数据
		   int i;
		   int index;
		  /* for(int i = 0; i < sizeof(myRecvData->frecenter); i++)
		   { 
			   float cfrecenter[i] = myRecvData->frecenter[i];
		   }*/

		   int cfrestep_50 = myRecvData->frestep_50;
		   int cfrestep_100 = myRecvData->frestep_100;
		   printf("需要测量的中心频率：%.1f, %.1f,\n 是否测试50GHz信道：%d\n 是否测试100GHz信道：%d\n", myRecvData->frecenter[0], myRecvData->frecenter[1], cfrestep_50, cfrestep_100);
		   WaitForSingleObject(g_hMutex, INFINITE);
		   if (cfrestep_50)
		   {
			   ...
			   printf("power:%.1f,%.1f\n", myData->osnr_db[0], myData->osnr_db[1]);
		   }
		   ReleaseMutex(g_hMutex);
		  
		   memcpy(sendbuffer, myData, needSend);
		   int position = 0;
